...
This is the groundwork for instrumenting the fixer (and later the organizer)
so every function that *chooses*, *alters*, or *scores* data can announce its
inputs and outputs when debugging is turned on, and cost nothing when it is off.

Design constraints (see scripts/audible-metadata-fixer-v5.py):

* The fixer's **stdout is parsed line-by-line** by app/main.py to drive the UI,
  so trace output must never go to stdout. It goes to a side channel (stderr by
  default, or a file via configure(file_path=...)).
* The fixer runs work on a ThreadPoolExecutor (up to ~20 workers), so the sink
  is lock-guarded and every line is tagged with the worker thread name.
* The fixer is also imported live by main.py and by the tests, so tracing is
  **disabled by default**: a decorated function behaves identically to the
  undecorated one until configure(enabled=True) is called.

Usage:

    from app.debug_trace import trace, trace_block, CHOOSE, ALTER, SCORE, subject
    ...
    with subject(folder_name):          # correlate all lines to one book
        ...
...
# Category tags. Free-form strings are allowed too, but these three cover the
# data-flow concerns we care about and can be filtered on independently.
CHOOSE = "choose"  # selects/picks among data (match selection, edit mode, query choice)
ALTER = "alter"    # transforms/sanitizes/normalizes/writes data
SCORE = "score"    # computes a score or ranking

_ENV_ENABLE = "LIBRAFORGE_DEBUG"
_ENV_FILE = "LIBRAFORGE_DEBUG_TRACE_FILE"
    ...
    def __init__(self, stream=None, file_path: Optional[str] = None) -> None:
        ...
        if file_path:
            try:
                ...
                # Never let a bad path break a real run; fall back to the stream.
                self._fh = None
...
def configure(
    ...
    *,
    ...
    """Turn tracing on/off and pick where it goes. Safe to call more than once.

    Environment fallbacks (only consulted when the matching argument is left at
    its default) let you flip tracing on without touching the CLI:
      LIBRAFORGE_DEBUG=1
      LIBRAFORGE_DEBUG_TRACE_FILE=/app/reports/run.trace.log
      LIBRAFORGE_DEBUG_CATEGORIES=score,choose
    """
    ...
    if _config.enabled:
        ...
        _config.sink.write(
            f"# debug-trace enabled  categories={sorted(_config.categories) if _config.categories else 'all'}"
        )
...
# --- subject correlation (which book/file are we processing) --------------------
...
def subject(value: Optional[str]):
    """Tag every trace line emitted in this block with `value` (e.g. folder name)."""
...
def _summarize(value: Any, max_len: int) -> str:
    """Compact, never-raising repr. Collapses big containers to shape, not content."""
    try:
...
def trace(
    ...
    *,
    ...
    """Decorate a data function to log its inputs/outputs when tracing is on.

    category   one of CHOOSE/ALTER/SCORE (or any string) for filtering.
    name       label to use (defaults to the function name).
    capture    arg names to log on entry. None => a compact view of all args.
               [] => log entry without args.
    show_result returns are logged unless False (use for huge/side-effect returns).
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            # Fast path: disabled tracing adds one attribute read + bool check.
            ...
            except Exception as exc:  # log then re-raise unchanged
...
def log(category: Optional[str], message: str, **fields) -> None:
    """Emit a one-off trace line (a checkpoint inside a function)."""
    if not is_enabled(category):
        return
    cat = category or "trace"
    if fields:
